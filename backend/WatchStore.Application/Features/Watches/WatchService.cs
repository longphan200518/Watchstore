        ...
        public async Task<ApiResponse<WatchDto>> UpdateAsync(UpdateWatchDto dto)
        {
            return await ExecuteInTransaction(async () =>
            {
                try
                {
                    ...
                    watch.Functions = dto.Functions;
                    // Kích thước
                    watch.Thickness = dto.Thickness;
                    watch.BandWidth = dto.BandWidth;
                    // Chất liệu
                    watch.Crystal = dto.Crystal;
                    watch.CaseMaterial = dto.CaseMaterial;
                    watch.BandMaterial = dto.BandMaterial;
                    // Khác
                    watch.WaterResistance = dto.WaterResistance;
                    watch.Warranty = dto.Warranty;
