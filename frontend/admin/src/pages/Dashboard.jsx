...
const stats = [
  { label: 'Tổng doanh thu', value: '₫3,2B', delta: '+12% MoM', color: 'text-green-500', bg: 'bg-green-50', icon: '💰' },
  { label: 'Đơn hàng', value: '1.248', delta: '+6% MoM', color: 'text-blue-500', bg: 'bg-blue-50', icon: '📦' },
  { label: 'Sản phẩm', value: '684', delta: '+18 mới', color: 'text-indigo-500', bg: 'bg-indigo-50', icon: '⌚' },
  { label: 'Khách hàng', value: '9.412', delta: '+3% MoM', color: 'text-amber-500', bg: 'bg-amber-50', icon: '👤' },
]

const activities = [
  { title: 'Đơn hàng #WS-1092', detail: 'Khách: Lê Minh Tuấn • 152.000.000₫', time: '5 phút trước', type: 'order' },
  { title: 'Thêm sản phẩm', detail: 'Royal Oak Offshore 44mm Ceramic', time: '23 phút trước', type: 'product' },
  { title: 'Cập nhật tồn kho', detail: 'Seamaster Aqua Terra +15 tồn', time: '1 giờ trước', type: 'stock' },
  { title: 'Đơn hàng #WS-1089', detail: 'Khách: Trần Khánh Linh • 92.500.000₫', time: '2 giờ trước', type: 'order' },
]

const quickLinks = [
  { label: 'Sản phẩm', path: '/products', desc: 'Tạo / chỉnh sửa sản phẩm', icon: '🛠️' },
  { label: 'Đơn hàng', path: '/orders', desc: 'Quản lý & xử lý đơn', icon: '🧾' },
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <main className="ml-64 p-10 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {stats.map((item) => (
            <div key={item.label} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-3">
              <div className="flex items-center justify-between">
                <div className={`w-11 h-11 rounded-lg ${item.bg} flex items-center justify-center text-lg`}>{item.icon}</div>
                <span className={`text-xs font-medium ${item.color}`}>{item.delta}</span>
              </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="divide-y divide-gray-100">
              {activities.map((act, idx) => (
                <div key={idx} className="py-3 flex items-start gap-3">
                  <span className="text-xl leading-none">{act.type === 'order' ? '🧾' : act.type === 'product' ? '🛠️' : '📦'}</span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">Truy cập nhanh</h3>
